 ...
 */
export function blockKey(name: string, sid: string): string {
  return sid !== '' ? sid : name;
}
 ...
 */
export function blockLabel(name: string, sid: string): string {
  if (name !== '') {
    return name;
  }
  return sid !== '' ? `<SID: ${sid}>` : '';
 ...
 */
export function joinBlockPath(parentPath: string, label: string): string {
 ...
 * name.
 */
export function isInsideBlockPath(ancestor: string, path: string): boolean {
  if (ancestor === '') {
