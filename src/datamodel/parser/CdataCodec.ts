 ...
 */
export function isMatCdata(value: unknown): boolean {
  if (value === null || typeof value !== 'object') {
    return false;
  }
  const v = (value as Record<string, unknown>)._value;
 ...
 */
export function uudecode(str: string): Uint8Array {
  const bits: number[] = [];
 ...
 */
export function uuencode(bytes: Uint8Array): string {
  const chars: string[] = [];
