...
interface Tag {
  ...
  dataOffset: number;
  totalSize: number;
  /** Whether the declared payload is entirely inside the view. */
  complete: boolean;
}
 ...
 */
function readTag(view: DataView, offset: number): Tag {
 ...
 */
function nameAt(view: DataView, base: number): string {
  let offset = base;

  const flags = readTag(view, offset);
  if (!flags.complete) throw new UnscannableMat('array flags subelement is truncated');
  ...
  if (arrayClass !== MX_OPAQUE) {
    ...
    if (!dims.complete) throw new UnscannableMat('dimensions subelement is truncated');
    offset += dims.totalSize;
  }

  const name = readTag(view, offset);
  if (!name.complete) throw new UnscannableMat('name subelement is truncated');
  if (name.type !== MI_INT8 && name.type !== MI_UINT8) {
    throw new UnscannableMat(`name subelement is type ${name.type}`);
  }
  const text = decoder.decode(
    new Uint8Array(view.buffer, view.byteOffset + name.dataOffset, name.bytes),
  );
 ...
 */
function scanNames(bytes: Uint8Array): string[] {
  if (bytes.length < 128) throw new UnscannableMat('shorter than the 128-byte header');
  const header = decoder.decode(bytes.subarray(0, 116)).trim();
  if (header.startsWith('MATLAB 7.3 MAT-file')) throw new UnscannableMat('v7.3 is HDF5');
  // 'IM', the little-endian indicator, byte by byte rather than decoded.
  if (bytes[126] !== 0x49 || bytes[127] !== 0x4d) throw new UnscannableMat('not little-endian');
 ...
 */
export function scanMat(bytes: ArrayBuffer): MatScanResult {
  try {
    ...
    if (!(err instanceof UnscannableMat)) throw err;
  }
