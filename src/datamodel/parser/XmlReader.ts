 ...
 * caller already does.
 */
export function readDictionaryXml(text: string): unknown {
 ...
 */
export function readModelXml(text: string): unknown {
  return defaultParser.parse(text);
}

/** One `<Info>` sidecar of a `.prj` store. Same options as a model part. */
export function readProjectXml(text: string): unknown {
  return defaultParser.parse(text);
}
