...
export interface MatVariable {
    ...
    _anonymous?: boolean;
    isOpaque?: boolean;
    // Why this variable's payload was NOT read, on a variable whose class the reader
    // recognized. Present only when the reader deliberately stopped, so `undecoded`
    // set is the difference between "this value is empty" and "this value exists and
    // this reader did not read it" — which `value: null` alone cannot say, and which
    // is the whole point of naming a class in CLASS_NAMES at all.
    //
    // A reason string rather than a boolean, and rather than a `warnings` array on
    // ParsedMat, for two reasons. The variable is where the fact belongs: a caller
    // holding one variable can tell what it is holding without consulting a
    // file-level list and matching by name. And the string is already the shape the
    // diagnostics channel wants — when parseMat gains warnings (docs/TODO.md item 3),
    // each of these becomes one `part-unreadable` with `part` set to the variable
    // name and this text as the message, with nothing here to change.
    //
    // When it is set, `value` holds the display placeholder below rather than null,
    // so a host that renders values without knowing about this field still says
    // something true.
    undecoded?: string;
}
...
export interface ParsedMat {
    ...
    // What this file CLAIMED and this reader could not read. ALWAYS an array, empty
    // when the file was read completely — never undefined, so a caller never has to
    // decide whether an absent field means "clean" or "this reader does not report".
    // ParsedProject.warnings set that precedent and this follows it deliberately.
    //
    // Note what is NOT in here: a variable of a class this file's release could not
    // have written, an empty file, a workspace with nothing in it. A `.mat` that ends
    // with the format's own zero terminator has ended, not been truncated. Everything
    // in this list is the file saying a variable is present and this reader coming
    // back without it — see test/parseWarnings.test.ts, which pins the near-misses as
    // firmly as the warnings.
...
// being the first.
function undecodedValue(dimensions: number[], className: string): string {
...
function readSparse(
    ...
    const dense: unknown[] = new Array(total);
...
export function parseMat(arrayBuffer: ArrayBuffer): ParsedMat {
    ...
    const nameFor = (variable: MatVariable) => (variable._anonymous ? '' : variable.name);
    ...
    while (offset < buf.length) {
        ...
        if (missing > 0) {
            ...
            if (shortened) { short.part = shortened; }
            warnings.push(short);
        }
...
function collectUndecoded(variable: MatVariable, path: string, warnings: ParseWarning[]): void {
    ...
    if (variable.className === 'cell' && Array.isArray(variable.value)) {
        const cells = variable.value as (MatVariable | null)[];
        ...
        cells.forEach((cell, i) => {
            if (cell) { collectUndecoded(cell, `${path}{${i + 1}}`, warnings); }
        });
    }
}
