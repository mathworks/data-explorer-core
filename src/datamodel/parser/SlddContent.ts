 ...
 */
export function isJsonTextBytes(bytes: Uint8Array): boolean {
  let i = 0;
...
export function readSlddContent(
  ...
  if (isJsonTextBytes(u8)) {
    return JSON.parse(strFromU8(u8)) as Record<string, unknown>;
  }
  return parseBinarySldd(bytes, warnings ?? []) as Record<string, unknown>;
}
