...
export default class AliasTypeNode extends SimulinkObjectNode {
    ...
    static createDefault(name: string, parent: BaseNode | null): AliasTypeNode { const rawVal = AliasTypeNode._defaultRawVal(CLASS_NAME, { BaseType: 'double', DataScope: 'Auto', Description: '', HeaderFile: '' }); const props = AliasTypeNode._propsOf(rawVal); return new AliasTypeNode(name, parent, props, { _rawVal: rawVal, _properties: props } as Record<string, unknown>); }
    static parse(rawVal: Record<string, unknown>, name: string, parent: BaseNode | null): AliasTypeNode { const props = AliasTypeNode._propsOf(rawVal); return new AliasTypeNode(name, parent, props, { _rawVal: rawVal, _properties: props } as Record<string, unknown>); }
}
