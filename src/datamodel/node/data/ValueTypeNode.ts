...
export default class ValueTypeNode extends SimulinkObjectNode {
    Description: string;
    DataType: string;
    // The rest of the value-property surface MATLAB models on Simulink.ValueType. These
    // fields are what made valueType.json's existing layout work: `min`/`max`/`unit` resolve
    // through schemaBridge's ATOM_BY_KEY to atoms that read node FIELDS, and the fields did
    // not exist — so each row rendered blank AND was added to shownKeys, which had the
    // "Other" catch-all suppress the raw key as well. A `"Unit": "m"` in the file was
    // unreachable in the UI, in both panes at once (test/parity/artifacts/text/params.sldd's
    // MyValueType is exactly that dictionary). Complexity {real|complex} and DimensionsMode
    // {Fixed|Variable} are closed enums and EDITABLE selects, validated by
    // DataNode._rejectUnknownEnumeral; Dimensions stays read-only for the reason
    // BusElementNode records — its constraint has not been worked out, and an unlock is
    // worth only as much as the rule that refuses a bad value.
    Dimensions: unknown;
    Complexity: string;
    ...
    constructor(name: string, parent: BaseNode | null, props: Record<string, unknown>, serial: Record<string, unknown>) {
        ...
        this.DataType = (props.DataType as string) || 'double';
        ...
        this.Unit = (props.Unit as string) || (props.DocUnits as string) || '';
    }
    ...
    setProperty(propName: string, stringValue: string): true | SetPropertyResult {
        if (propName === 'Min' || propName === 'Max') { return this._setMinMax(propName, stringValue); }
        const notAnEnumeral = this._rejectUnknownEnumeral(propName, stringValue);
        if (notAnEnumeral) { return notAnEnumeral; }
    ...
    _serializedOverrides(): Record<string, unknown> {
        ...
        if ('Complexity' in sp || (this.Complexity && this.Complexity !== 'real')) { overrides.Complexity = this.Complexity; }
        if ('DataType' in sp || this.DataType !== 'double') { overrides.DataType = this.DataType; }
        Object.assign(overrides, this._gatedProps({ Description: this.Description }));
        if ('DimensionsMode' in sp || (this.DimensionsMode && this.DimensionsMode !== 'Fixed')) { overrides.DimensionsMode = this.DimensionsMode; }
        ...
        if ('Max' in sp || this.Max !== undefined) { overrides.Max = this.Max !== undefined ? this.Max : []; }
        if ('Min' in sp || this.Min !== undefined) { overrides.Min = this.Min !== undefined ? this.Min : []; }
        if (unitKey in sp || this.Unit) { overrides[unitKey] = this.Unit; }
        return overrides;
    }
    static get defaultName(): string { return 'ValueType'; }
    static createDefault(name: string, parent: BaseNode | null): ValueTypeNode { const rawVal = ValueTypeNode._defaultRawVal(CLASS_NAME); const props = ValueTypeNode._propsOf(rawVal); return new ValueTypeNode(name, parent, props, { _rawVal: rawVal, _properties: props } as Record<string, unknown>); }
    static parse(rawVal: Record<string, unknown>, name: string, parent: BaseNode | null): ValueTypeNode { const props = ValueTypeNode._propsOf(rawVal); return new ValueTypeNode(name, parent, props, { _rawVal: rawVal, _properties: props } as Record<string, unknown>); }
}
