 ...
 */
export function owningEntryOf(node: BaseNode | null | undefined): DataNode | null {
  let entry: BaseNode | null | undefined = node;
  while (entry && !(entry as DataNode).isEntry) {
    entry = entry.parent;
  }
  return (entry as DataNode | null | undefined) ?? null;
}
...
export default class DataNode extends BaseNode {
  ...
  setProperty(propName: string, stringValue: string): true | SetPropertyResult {
    ...
    if (resolved === 'name') {
      ...
      const entryRenamed = (this.parent as unknown as { _entryRenamed?: (from: string, to: string) => void })
        ?._entryRenamed;
    ...
    if (resolved === 'Description' && !this.descriptionEditable) {
      return {
        ...
        invalidValue: stringValue,
        validValue: (this as unknown as { Description?: string }).Description || '',
      };
    }
  ...
  _markModified(): void {
    ...
    for (let node: BaseNode | null = this; node && node !== entry; node = node.parent) {
  ...
  static serializePropertyXml(name: string, value: unknown, indent: number, ownerNode: DataNode | null): string {
    ...
    if (typeof value === 'object') {
      ...
      if (obj._array_type === 'String') {
        const elements = (obj._elements as unknown[]) || [];
        return DataNode._serializeStringPropertyXml(
          name,
          elements,
          (obj._dimensions as number[]) || [1, elements.length],
          indent,
        );
      }
    }
  ...
  static _stringEnvelopeXml(elements: unknown[], dims: number[], indent: number): string {
    ...
    for (const el of elements) {
      ...
      xml += ip3 + '<Element Class="char">' + escapeXml((el as string) || '') + '</Element>\n';
    }
    xml += ip2 + '</P>\n';
    xml += ip + '</Element>\n';
    return xml;
