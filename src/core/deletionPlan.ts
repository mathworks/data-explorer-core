...
import type BaseNode from '../datamodel/node/BaseNode.js';
...
function hasSelectedAncestor(node: DataNode, selected: ReadonlySet<BaseNode>): boolean {
  for (let n: BaseNode | null = node?.parent; n; n = n.parent) {
...
export function planDeletion(
  ...
  findNode: (rowId: string) => DataNode | null,
): DeletionPlan {
  ...
  const groupOf = new Map<DataNode, ChildGroup>();
