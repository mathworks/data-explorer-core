...
import type { MaskScope } from '../datamodel/maskScope.js';
 ...
 * know.
 */
function mutateSubtree<T>(root: INode, mutate: () => T): T {
  ...
  try {
    ...
    const survivors = new Set<INode>(after);
...
function blockWithKey(srcId: string, key: string): INode | null {
  const source = dataSources.get(srcId);
  const section =
    source && typeof (source as { getSection?: unknown }).getSection === 'function'
      ? ((source as unknown as { getSection: (k: string) => INode | null }).getSection('blocks') as INode | null)
      : null;
  if (!section || !Array.isArray(section.children)) {
    return null;
  }
  for (const child of section.children) {
    const sid = (child as unknown as { sid?: unknown }).sid;
...
function dictionaryRefNamesOf(srcId: string): string[] {
  const source = dataSources.get(srcId);
  if (!source) {
    return [];
  }
  return normalizeRefNames((source as unknown as { dictionaryReferences?: unknown }).dictionaryReferences);
}
...
function resolutionOrder(model: ISourceNode, modelSrcId: string): string[] {
  // Index 0 is the model itself, which is how its own workspace comes first.
  const order: string[] = [modelSrcId];
...
function definedNamesOf(srcId: string, asModel: boolean): Set<string> {
  ...
  if (asModel) {
    ...
    for (const child of childrenOf(workspace as INode | null)) {
      add(child);
    }
    return names;
  }
...
function collectUsages(definitions: INode[]): Map<string, NodeUsage[]> {
  ...
  for (const [srcId, source] of dataSources) {
    ...
    const declaredMasks = (source as unknown as { masks?: unknown }).masks;
    const masks: MaskScope[] = Array.isArray(declaredMasks)
      ? (declaredMasks as MaskScope[]).filter(
          (mask) => mask && Array.isArray(mask.names) && typeof mask.blockPath === 'string',
        )
      : [];
    ...
    const definedAt: (Set<string> | null)[] = ranked.map(() => null);
    const namesAt = (i: number): Set<string> => {
    ...
    for (const [name, defs] of byName) {
      ...
      const winners = defs.filter(
        (def) => definitionRank(source, def.node, def.owner, def.srcId, rankOf) === resolved,
      );
      if (winners.length > 0) {
