...
## A defect found by clicking a link the Usage column drew

Also not a phase, and reported the same way 49 was — by a user, on their own models. Three
parameters in `mWhereUsedRefVars.slx` and its child rendered as links and did nothing when
clicked: a Bus Selector's `OutputSignals=a,b`, a Math Function's `Operator=square`, and a
Model block's file name. The dead link is a **VS Code renderer** defect and is fixed there
(both `paramLinks` arms now gate the anchor on `linkTarget`, as the Value column already
did). The rows themselves are this repo's, and two of the three should never have existed.

51. **The `<P>` gate admitted parameters that cannot hold data at all.** `isParamReference`
    was a property blocklist plus a value test, and neither can see the difference between
    `Gain=square` and `Operator=square`: `square` is a legal variable name, so the value
    test must say yes. What rules it out is which parameter holds it — `Operator` is an
    option list of fifteen choices — and that is a fact about the **block type**, which
    the gate never had.
    ...
    users at all, and our index put the Bus Selector on it: a **false reverse edge**, and
    one a user cannot tell from a true one. It is the same trap `EXPRESSION_MASK_TYPES`
    was built to avoid, arriving by the one path that had no such table.

    Fixed by keying the gate on the `(BlockType, parameter)` PAIR against two measured
    tables: `ENUM_BLOCK_PARAMS`, 343 option-list pairs generated by `gen_block_params.m`
    from `DialogParameters`, and a three-entry hand list of free-text parameters
    `Simulink.findVars` measurably never credits (`probe_non_data_params.m`). Two other
    rules were tried and refuted first, both failing by hiding real references — see [The
    block-parameter gate](#the-block-parameter-gate) in the README, which records them so
    they are not re-derived. A block type in neither table is judged exactly as before,
    which is the direction issue #9 argues for.

    Held by `blockParamUsages.test.ts` (7 of its cases fail with the fix reverted),
    including the pair-keying itself — `Operator` suppressed on a Math block and kept on a
    Gain — the mask parameters the gate must not reach, and the `.slx` and `.mdl` of the
    same three blocks held against **each other** rather than against a literal, because
    one rule spelled in two parsers is the shape this repo's bugs keep taking.

    Two things the same measurement settled and left alone. A Model block's
    `ParameterArgumentValues` **is** a data reference — a model argument is an expression in
    the parent's scope — so it stays admitted, one parameter away from the file name that
    does not. And `Gain=finalGain` in the reported model is a real reference that findVars
