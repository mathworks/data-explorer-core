...
| `probe_cell_shapes.m` | how does MATLAB spell a shaped value INSIDE a cell — a column, a matrix, a logical, a char matrix, a string — and in which order does it store the cell's own elements? (defects 49, 50) | `$CELL_SHAPES_OUT` or `tempdir/cellshapes` | — |
| `probe_cell_string.m` | and how does it spell a `string` in a cell in the BINARY flavour, where a string is an object rather than a JSON value? (defect 52) | `$CELL_STRING_OUT` or `tempdir/cellstring` | `cellstr_text.sldd`, `cellstr_binary.sldd` |
| `probe_cell_arrays.m` | and how does it spell an ARRAY in a cell — complex, struct, object — where the cell site's hand-rolled dispatch lagged the shared one by three ideas? (defects 53, 54, 55) | `$CELL_ARRAYS_OUT` or `tempdir/cellarrays` | `cellarr_text.sldd`, `cellarr_binary.sldd` |
