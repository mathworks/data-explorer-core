...
// `import { x } from './y.js'` does all three. This package leans on that difference
// deliberately and heavily: the node layer is a deep class hierarchy in which
// `BaseNode` needs `schemaBridge`'s layout builder as a VALUE while `schemaBridge`
// and fifteen `Prop*` atoms need `BaseNode` only as a TYPE. Written the other way
// round, `class PropName extends ...` would evaluate against a half-initialised
// module and throw at load — a total failure, not a subtle one. A graph walker that
// cannot see the `type` keyword reports a dozen cycles here that do not exist, so
