...
export function parseBinarySldd(arrayBuffer) {
...
export function parseBinarySlddParts(xmlString, zipMetadata) {
    const decoder = new TextDecoder();
    let release = '';
    ...
    const dataSource = doc.DataSource;
    ...
    if (elements && elements.length > 0 && elements[0]['@_Class']) {
        ...
        return {
            ...
            _elements: elements.map((el) => {
                ...
                const inner = parsed._elements[0];
...
function transposeColumnMajor(values, dims) {
    const rows = dims[0];
    const cols = dims[1];
    if (rows <= 1) {
        return values;
...
function parsePropContent(prop, handleStructClass = true) {
