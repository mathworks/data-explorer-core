...
export function configSetIdentity(data) {
    ...
    const obj = typeof rec['@_ClassName'] === 'string'
        ? rec
        : findAll(rec, 'Object')[0];
 ...
 */
export function isParamReference(blockType, propName, value) {
    if (!propName || NON_PARAM_PROPS.has(propName))
        return false;
    if (NON_DATA_PARAMS.get(blockType)?.has(propName))
        return false;
    ...
    const collect = (system, path, descend) => {
        for (const block of findAll(system, 'Block')) {
            ...
            for (const p of props ? (Array.isArray(props) ? props : [props]) : []) {
                ...
                const val = pObj['#text'] || '';
                if (!isParamReference(blockType, propName, val))
                    continue;
    ...
    if (entries['simulink/modelWorkspace.mxarray']) {
        ...
        if (workspace.length === 0) {
            ...
            if (!outer || !outer.fields) {
                warnings.push({
                    code: 'part-unreadable',
                    message: 'The model part "simulink/modelWorkspace.mxarray" does not hold a decodable '
        ...
        try {
            ...
            warnings.push({
                code: 'part-unreadable',
                message: `The model part "simulink/modelworkspace.mat" could not be read (${reasonOf(err)}), `
