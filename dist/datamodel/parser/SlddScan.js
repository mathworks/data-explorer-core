...
function indexOfBytes(hay, needle, from, skip) {
...
function matchesAt(hay, at, text) {
    if (at < 0 || at + text.length > hay.length)
        return false;
...
function skipSpace(hay, at) {
    let i = at;
    while (i < hay.length && isSpace(hay[i]))
        i++;
    return i;
}
// ---------------------------------------------------------------------------------
 ...
 */
export function scanDataSourceXml(xml) {
    ...
    while (open >= 0 || close >= 0) {
        if (close >= 0 && (open < 0 || close < open)) {
            depth--;
            if (depth < 0)
                throw new UnscannableDictionary('unbalanced </Object>');
...
function seekByte(xml, from, byte) {
    let i = from;
 ...
 */
function firstPropertyText(xml, from, want) {
    ...
    if (end >= xml.length)
        throw new UnscannableDictionary('unterminated text node');
    if (xml[end + 1] === BANG) {
 ...
 */
export function scanSldd(bytes, warnings) {
    ...
    if (!isJsonTextBytes(u8)) {
        ...
        if (xml) {
            try {
                ...
                if (!(e instanceof UnscannableDictionary))
                    throw e;
            }
        }
    }
...
//# sourceMappingURL=SlddScan.js.map