...
// analog of the JSON entry splicer, which is still in the consumer because it needs a
// JSON parser this package does not depend on. No value parsing. Relies on the verified
// invariant that an <Object Class="DD.ENTRY">…</Object> fragment never contains a nested
// <Object> (nested objects serialize as <Element Class="...">), so a linear scan of the
// entry open/close tags is unambiguous. Never throws; returns null when not found.
import { toEntrySelector } from './entrySelector.js';
 ...
 */
export function findEntryObjectSpan(xml, target) {
    ...
    for (;;) {
        ...
        if (entryNameOf(fragment) === selector.name) {
            ...
            if (!selector.uuid)
                return span;
            if (entryUuidOf(fragment) === selector.uuid)
                return span;
            firstMatch ??= span;
        }
        pos = endExclusive;
 ...
 */
export function findEntryElementSpan(xml, target) {
    ...
    if (xml.slice(lineStart, start).trim() === '')
        start = lineStart;
 ...
 */
export function findEntryInsertionPoint(xml) {
...
//# sourceMappingURL=xmlEntrySplice.js.map